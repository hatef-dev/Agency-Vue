  ...
  <div class="flex flex-col gap-y-4 lg:gap-y-10 my-container">
    ...
    <transition-animation>
      <div class="grid grid-cols-2 gap-6">
        <div v-for="project in projects" :key="project.title" class="relative group">
          <img :src="project.image" :alt="project.title" class="rounded-2xl" />
          <div
            class="absolute -z-0 transition-all flex justify-center items-center opacity-0 group-hover:opacity-100 duration-200 ease-in-out backdrop-blur-xs brightness-50 w-full h-full left-0 top-0 rounded-2xl"
          ></div>
          <div class=""></div>
        </div>
      </div>
